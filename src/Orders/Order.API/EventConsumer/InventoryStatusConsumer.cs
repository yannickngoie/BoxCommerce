...
namespace Order.API.EventConsumer
{
    public class InventoryStatusConsumer : IConsumer<InventoryUpdateEvent>
    {
        ...
        public async Task Consume(ConsumeContext<InventoryUpdateEvent> context)
        {
            ...
            _logger.LogInformation("Inventory Update for  Work Item  with {OrderId} :", command.Id + " with Order status as " + command.OrderStatus + " sent successfully " + ", CorrelationID " + eventMessage.CorrelationId);
        }
